...
def get_dashboard_data():
    ...
    # --- NOVOS PARÂMETROS DA REQUISIÇÃO ---
    metric = request.args.get('metric', 'Valor Líquido')
    # Recebe o parâmetro 'top_n', com 20 como padrão. Converte para inteiro.
    ...
    # --- CÁLCULO DOS KPIs DINÂMICOS (sem alteração) ---
    ...
    # Se top_n for -1 (Todos) ou se o número de entes for menor que o top_n pedido, mostra todos.
    if top_n == -1 or len(df_sorted) <= top_n:
        chart_labels = df_sorted['Nome do Ente'].tolist()
        chart_values = df_sorted[metric].tolist()
    else: # Caso contrário, aplica a lógica "Top N + Outros"
