...
#%% testando com o p´roprio "df_final"

# ==============================================================================
# SCRIPT DE PRODUÇÃO v2: FOCO NO CLIENTE E CAPACIDADE OPERACIONAL
# ==============================================================================
import pandas as pd
import numpy as np
import joblib
...
# Capacidade Operacional: Quantos clientes a equipe de cobrança consegue acionar?
# Este é o número de clientes que queremos no nosso relatório de ação.
CAPACIDADE_COBRANCA = 500


# --- ETAPA 1: CARREGAR O MODELO TREINADO (sem alterações) ---
...
# --- ETAPA 3: EXECUÇÃO E AGREGAÇÃO POR CLIENTE ---
print("\n--- ETAPA 2: Executando o processo de predição e agregação ---")

if 'df_final' not in locals() or df_final.empty:
    print("AVISO: DataFrame 'df_final' não encontrado ou está vazio.")
else:
    # 1. Pré-processa os dados
    df_predict = preprocess_new_data(df_final)
    
    if not df_predict.empty:
        # 2. Gera a probabilidade para CADA PARCELA
        print("Gerando previsões de probabilidade para cada parcela...")
        probabilities = production_model.predict_proba(df_predict)[:, 1]
        df_predict['Probabilidade_Inadimplencia'] = probabilities
        
        # 3. AGREGAÇÃO POR CLIENTE (A GRANDE MUDANÇA)
        print("Agregando resultados por cliente para uma visão consolidada...")
        ...
        # Ordena os CLIENTES pelo seu risco máximo
        df_agregado_cliente_sorted = df_agregado_cliente.sort_values(by='Risco_Maximo_Cliente', ascending=False)
        
        # 4. GERA O RELATÓRIO FINAL BASEADO NA CAPACIDADE
        print("Montando o relatório final de ação focado na capacidade operacional...")
        
        # Pega apenas os Top N clientes definidos na capacidade
        df_acao_final = df_agregado_cliente_sorted.head(CAPACIDADE_COBRANCA).copy()
        df_acao_final['Acao_Recomendada'] = 'ACIONAR COBRANÇA PRIORITÁRIA'
        
        # Formata os valores para melhor visualização
        df_acao_final['Valor_Total_Devido'] = df_acao_final['Valor_Total_Devido'].map('R$ {:,.2f}'.format)
        
        # 5. Apresenta os resultados
        print("\n\n==========================================================")
        print("RELATÓRIO DE RISCO POR CLIENTE (FOCO OPERACIONAL)")
        print("==========================================================")
        
        total_clientes_analisados = len(df_agregado_cliente_sorted)
        
        print(f"Total de clientes únicos analisados: {total_clientes_analisados}")
        print(f"Relatório focado nos Top {CAPACIDADE_COBRANCA} clientes de maior risco.")
        
        print("\nLista de Clientes para Ação Prioritária:")
        display(df_acao_final)
        
    else:
        print("Nenhum dado válido para predição após o pré-processamento.")
...
# ==============================================================================
# ANÁLISE DE RISCO SISTÊMICO DA CARTEIRA
# ==============================================================================

if 'df_predict' in locals() and not df_predict.empty:
    print("--- ANÁLISE DE RISCO SISTÊMICO DA CARTEIRA ---")

    df_predict['Valor_Esperado_Perda'] = df_predict['Probabilidade_Inadimplencia'] * df_predict['ValorNominal']
    valor_total_carteira = df_predict['ValorNominal'].sum()
    risco_sistemico_total = df_predict['Valor_Esperado_Perda'].sum()
    percentual_risco = (risco_sistemico_total / valor_total_carteira) * 100

    print(f"\nValor Total da Carteira Analisada: R$ {valor_total_carteira:,.2f}")
    print(f"Valor Esperado da Perda (Risco Total): R$ {risco_sistemico_total:,.2f}")
    print(f"Percentual de Risco da Carteira: {percentual_risco:.2f}%")
    risco_por_convenio = df_predict.groupby('Convênio').agg(
        Valor_Total_Segmento=('ValorNominal', 'sum'),
        Risco_Total_Segmento=('Valor_Esperado_Perda', 'sum')
    ).reset_index()

    risco_por_convenio['Percentual_Risco_Segmento'] = (risco_por_convenio['Risco_Total_Segmento'] / risco_por_convenio['Valor_Total_Segmento']) * 100
    risco_por_convenio_sorted = risco_por_convenio.sort_values(by='Risco_Total_Segmento', ascending=False)
    
    print("\nDetalhamento do Risco por Convênio (Top 10):")
    risco_por_convenio_sorted['Valor_Total_Segmento'] = risco_por_convenio_sorted['Valor_Total_Segmento'].map('R$ {:,.2f}'.format)
    risco_por_convenio_sorted['Risco_Total_Segmento'] = risco_por_convenio_sorted['Risco_Total_Segmento'].map('R$ {:,.2f}'.format)
    
    display(risco_por_convenio_sorted.head(10))

else:
    print("DataFrame 'df_predict' não encontrado. Execute o script de predição primeiro.")
