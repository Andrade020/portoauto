...
if __name__ == "__main__":
    ...
    if any(t in DADOS_A_BAIXAR for t in TAREFAS_BLOCO_1):
        criar_pastas_principais()
        if 'cvm' in DADOS_A_BAIXAR: baixar_informes_mensais_fidc_cvm()
        if 'bcb' in DADOS_A_BAIXAR: baixar_series_temporais_bcb()
        if 'ibge_pnad' in DADOS_A_BAIXAR: baixar_dados_pnad_ibge()
        if 'tesouro' in DADOS_A_BAIXAR: baixar_dados_tesouro_direto()
    if any(t in DADOS_A_BAIXAR for t in TAREFAS_BLOCO_2):
        criar_pastas_adicionais()
        if 'ibge_mensal' in DADOS_A_BAIXAR: chamar_downloads_ibge_mensal()
        if 'ibge_pib' in DADOS_A_BAIXAR: baixar_pib_municipios_ibge()
        if 'caged' in DADOS_A_BAIXAR: baixar_novo_caged()
        if 'tse' in DADOS_A_BAIXAR: baixar_perfil_eleitorado_tse()
        if 'inmet' in DADOS_A_BAIXAR: baixar_dados_climaticos_inmet()
    print("\n" + "="*60 + "\n>>> ROTINAS SELECIONADAS CONCLUÍDAS <<<\n" + "="*60)