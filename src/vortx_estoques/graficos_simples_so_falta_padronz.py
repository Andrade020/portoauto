...
def plotar_aging_com_acumulado(df_estoque, col_valor='Valor Presente'):
    ...
    if df_vencidos.empty: return None
    ...
    # Lógica de posicionamento da linha (simplificada)
    ax.set_ylim(0, total_em_atraso * 1.4)
    ax2.set_ylim(0, 1.4) 
    ...
    # Rótulos das barras (em milhares)
    for bar in bars:
        height = bar.get_height()
        label = f'{height/1000:,.0f}'.replace(',', '.')
        ax.text(bar.get_x() + bar.get_width() / 2, height, label,
